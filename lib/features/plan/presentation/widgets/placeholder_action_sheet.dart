...
class _PlaceholderActionSheetState
  ...
  /// 食事タイプを抽出（ランチ、ディナー、居酒屋など）
  String _extractMealType(String location) {
    final lower = location.toLowerCase();
    if (lower.contains('ランチ') || lower.contains('昼食')) {
      return 'ランチ';
    } else if (lower.contains('ディナー') || lower.contains('夕食')) {
      return 'ディナー';
    } else if (lower.contains('朝食')) {
      return 'モーニング';
    } else if (lower.contains('カフェ')) {
      return 'カフェ';
    } else {
      return 'レストラン';
    }
  ...
  List<Widget> _buildQuickActionChips() {
    ...
    switch (_type) {
      case PlaceholderType.accommodation:
        actions.addAll([
          _QuickAction(
            ...
            onTap:
                () => _openUrl(
                  PlaceholderHelper.getSearchUrl(_type, keyword, area: area),
                ),
          ),
          _QuickAction(
            ...
            onTap:
                () => _openUrl(
                  'https://travel.rakuten.co.jp/keyword/${Uri.encodeComponent(area.isNotEmpty ? area : keyword)}/',
                ),
          ),
          _QuickAction(
            icon: Icons.map,
            ...
            onTap:
                () => _openUrl(
                  'https://www.google.com/maps/search/${Uri.encodeComponent('$area ホテル 旅館')}',
                ),
          ),
        ]);
        break;
      ...
      case PlaceholderType.meal:
        // 食事タイプを抽出（ランチ、ディナーなど）
        final mealType = _extractMealType(widget.item.location);
        // 検索クエリ用の地域とタイプを組み合わせる
        final searchArea = area.isNotEmpty ? area : keyword;
        final mealSearch = '$searchArea $mealType';
        ...
        actions.addAll([
          _QuickAction(
            ...
            onTap:
                () => _openUrl(
                  'https://tabelog.com/rstLst/?vs=1&sw=${Uri.encodeComponent(searchArea)}',
                ),
          ),
          _QuickAction(
            ...
            onTap:
                () => _openUrl(
                  'https://www.hotpepper.jp/SA11/search/?keyword=${Uri.encodeComponent(searchArea)}',
                ),
          ),
          _QuickAction(
            icon: Icons.map,
            ...
            onTap:
                () => _openUrl(
                  'https://www.google.com/maps/search/${Uri.encodeComponent(mealSearch)}',
                ),
          ),
        ]);
        break;
      ...
      case PlaceholderType.shopping:
        actions.addAll([
          _QuickAction(
            ...
            onTap:
                () => _openUrl(
                  'https://www.google.com/search?q=${Uri.encodeComponent('$area 観光 おすすめ')}',
                ),
          ),
          _QuickAction(
            icon: Icons.map,
            ...
            onTap:
                () => _openUrl(
                  'https://www.google.com/maps/search/${Uri.encodeComponent('$area 観光')}',
                ),
          ),
          _QuickAction(
            ...
            onTap:
                () => _openUrl(
                  'https://www.jalan.net/kankou/spt_guide/?screenId=OUW6010&keyword=${Uri.encodeComponent(area.isNotEmpty ? area : keyword)}',
                ),
          ),
        ]);
        break;
      ...
      case PlaceholderType.transport:
        actions.addAll([
          _QuickAction(
            ...
            onTap:
                () => _openUrl(
                  'https://www.google.com/maps/dir/?api=1&destination=${Uri.encodeComponent(keyword.isNotEmpty ? keyword : area)}',
                ),
          ),
          _QuickAction(
            icon: Icons.train,
            ...
            onTap:
                () => _openUrl(
                  'https://transit.yahoo.co.jp/search/result?to=${Uri.encodeComponent(keyword.isNotEmpty ? keyword : area)}',
                ),
          ),
          _QuickAction(
            ...
            onTap:
                () => _openUrl(
                  'https://www.google.com/search?q=${Uri.encodeComponent('$area 高速道路 ルート')}',
                ),
          ),
          _QuickAction(
            ...
            onTap:
                () => _openUrl(
                  'https://www.google.com/search?q=${Uri.encodeComponent('$area タクシー 料金')}',
                ),
          ),
        ]);
        break;
      ...
      case PlaceholderType.other:
        ...
        actions.add(
          _QuickAction(
            ...
            onTap:
                () => _openUrl(
                  'https://www.google.com/maps/search/${Uri.encodeComponent('$area $keyword')}',
                ),
          ),
        );
    }
